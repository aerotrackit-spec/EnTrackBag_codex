...
public sealed class BagJourneyConfigurationController : ControllerBase
{
    ...
    public BagJourneyConfigurationController(IBagJourneyConfigurationDomainComponent domainComponent)
    {
        _domainComponent = domainComponent;
    }

    [HttpGet]
    public async Task<ActionResult<BagJourneyConfigurationDto>> Get(CancellationToken ct)
        => Ok(await _domainComponent.GetAsync(ct));
    ...
    public async Task<ActionResult<BagJourneyConfigurationDto>> Update(UpdateBagJourneyConfigurationDto request, CancellationToken ct)
    {
        try
        {
            return Ok(await _domainComponent.UpdateAsync(request, ct));
        }
